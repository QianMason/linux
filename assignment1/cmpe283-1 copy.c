...
uint32_t output_eax, output_ebx, output_ecx, output_edx, function_number;
...
init_module(void)
{
	...
	// detect_vmx_features();
	int a, b, c, d;
		if (!__get_cpuid (0x80000001 /* nonexistent, but assume it exists */, &a, &b, &c, &d))
		{
	  		printf (stderr, "Warning: CPUID request 0x80000001 not valid!\n");
		}
	printf("EAX: %x\nEBX: %x\nECX: %x\nEDX: %x\n", a, b, c, d);
	return 0;
